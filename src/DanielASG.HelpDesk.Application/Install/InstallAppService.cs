...
namespace DanielASG.HelpDesk.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : HelpDeskAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<HelpDeskDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{HelpDeskConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{HelpDeskConsts.ConnectionStringName}", constring);
        }
