...
{
    [AbpAuthorize(AppPermissions.Pages_Administration_Users, AppPermissions.Pages_Tickets)]
    public class UserAppService : HelpDeskAppServiceBase, IUserAppService
    {
        ...
        public UserAppService(
            ...
            IRepository<UserDepartment, int> userDepartmentRepository,
        ...
        public async virtual Task<BaseSelectInputDto[]> GetAllForSelectInput(string filterText, long?[] initialId)
        {
            var query = _userRepository.GetAll()
                                       .AsNoTracking()
                                       .WhereIf(initialId != null && initialId.Any(), x => initialId.Contains(x.Id))
                                       .WhereIf(!filterText.IsNullOrEmpty(),
                                        a => a.EmailAddress.Contains(filterText) ||
        ...
        public async Task<GetUserForEditOutput> GetUserForEdit(NullableIdDto<long> input)
        {
            ...
            output.User.DepartmentIds = await _userDepartmentRepository.GetAll()
        ...
        public async Task DeleteUser(EntityDto<long> input)
        {
            ...
            var userDepartments = await _userDepartmentRepository.GetAll()
                                                                 .Where(x => x.UserId == input.Id)
                                                                 .Select(x => x.Id)
                                                                 .ToListAsync();
        ...
        protected virtual async Task UpdateUserAsync(CreateOrUpdateUserInput input)
        {
            ...
            if (user.ProfileType == EProfileType.Staff || user.ProfileType == EProfileType.Customer)
            {
                var userDepartments = await _userDepartmentRepository.GetAll()
                ...
                if (!input.User.DepartmentIds.IsNullOrEmpty())
                {
                    foreach (var departmentId in input.User.DepartmentIds)
                    {
        ...
        [AbpAuthorize(AppPermissions.Pages_Administration_Users_PasswordlessToken)]
        public async Task<string> GeneratePasswordlessTokenAsync(long userId)
        {
            var user = await _userRepository.GetAll().Include(x => x.Roles).Where(x => x.Id == userId).FirstOrDefaultAsync();
        ...
        private IQueryable<User> GetUsersFilteredQuery(IGetUsersInput input)
        {
            var query = UserManager.Users
                ...
                .WhereIf(input.ProfileType == EProfileType.Staff, u => u.ProfileType == EProfileType.Staff)
                .WhereIf(input.ProfileType == EProfileType.Customer, u => u.ProfileType == EProfileType.Customer)
