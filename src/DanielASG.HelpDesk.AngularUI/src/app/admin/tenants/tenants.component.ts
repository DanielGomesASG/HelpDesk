...
export class TenantsComponent extends AppComponentBase implements OnInit {
    ...
    _entityTypeFullName = 'DanielASG.HelpDesk.MultiTenancy.Tenant';
    ...
    resetDataBuilder(record: any): void {
        this._tenantService.resetTenantDataBuilder(new EntityDtoOfInt64({ id: record.id })).subscribe(() => {
            this.notify.success(this.l('ResetDataBuilderSuccess', record.name));
        });
    }
