...
export class UsersComponent extends AppComponentBase implements AfterViewInit {
    ...
    showDynamicProperties(user: UserListDto): void {
        this.dynamicEntityPropertyManager
            .getModal()
            .show('DanielASG.HelpDesk.Authorization.Users.User', user.id.toString());
    }
    ...
    profileType = EProfileType.Internal;
    activeTab = 0;
    switchHeaders(event?: any) {
        this.activeTab = event.index;

        if (this.activeTab === 0)
            this.profileType = EProfileType.Internal;

        if (this.activeTab === 1)
            this.profileType = EProfileType.Staff;

        if (this.activeTab === 2)
            this.profileType = EProfileType.Customer;
