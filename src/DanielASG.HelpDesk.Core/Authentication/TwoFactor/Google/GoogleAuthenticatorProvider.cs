...
namespace DanielASG.HelpDesk.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : HelpDeskServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("DanielASG.HelpDesk", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
